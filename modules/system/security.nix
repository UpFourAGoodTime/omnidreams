  ...
  flake.nixosModules.security-config =
    ...
    }:
    {
      ...
      boot.kernel.sysctl = {
        ...
        # Raised the vm.max_map_count substantially to accommodate the very large number of guard pages created by hardened_malloc.
        "vm.max_map_count" = 1048576;

        # Limits for how many minor mistakes can occur before a panic/warning
      ...
      # See https://github.com/GrapheneOS/hardened_malloc
      ...
      security.apparmor.includes = {
        "abstractions/base" = ''
          ...
          /nix/store/*/lib/** mr,
          /nix/store/** r,
          ${lib.getExe' pkgs.coreutils "coreutils"} rix,
          ${lib.getExe' pkgs.coreutils-full "coreutils"} rix,
        '';
      };
      ...
      environment.systemPackages = [
        (pkgs.writeShellApplication {
          ...
          text = ''
            mkdir -p /tmp/check-kernel-hardening
            sysctl -a > /tmp/check-kernel-hardening/params.txt
            kernel-hardening-checker -l /proc/cmdline -c /proc/config.gz -s /tmp/check-kernel-hardening/params.txt
            rm -rf /tmp/check-kernel-hardening
          '';
        })
      ];
